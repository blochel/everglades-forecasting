...
ggplot(mapping = aes(x = year, y = count)) +
    ...
    scale_color_discrete(labels = c("arima_exog", "arima", "tslm"))

ggsave("results/everglades-models.png", height = 6, width = 5)
```

### Forecasts

```{r}
# Test-train split: hold out last 5 years
train_data <- everglades_count_env_data |>
  filter(year <= max(year) - 5)

test_data <- everglades_count_env_data |>
  filter(year > max(train_data$year))
...
forecasts <- train_models |>
  forecast(new_data = test_data)

# Evaluate forecast accuracy
accuracy_results <- accuracy(forecasts, everglades_count_env_data)
print(accuracy_results)

autoplot(forecasts, train_data, level = 80, alpha = 0.5) +
  autolayer(test_data, count, color = 'black')

ggsave("results/everglades-forecasts.png", height = 6, width = 5)
```
