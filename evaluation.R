...
# Optimized with parallel processing and efficient data handling
# =============================================================================
...
fit_sliding_window <- function(data,
                               ...
                               test_years,
                               cv_windows = NULL,
                               parallel = FALSE,  # Default to FALSE for now
                               workers = NULL,
                               ...) {
  ...
  # Sequential processing for now (more stable)
  ...
  results_list <- lapply(seq_along(train_starts), function(i) {
    ...
    list(
      forecasts = forecast_and_metrics[[1]] |> mutate(test_start = test_starts[i], window = i) |> as_tibble(),
      metrics = forecast_and_metrics[[2]] |> mutate(test_start = test_starts[i], window = i) |> as_tibble()
    )
  })
...
calculate_rps_mvgam <- function(predictions, test_data, train_data, config, precomputed_breaks = NULL) {
  ...
  forecasts_probs <- predictions |>
    ...
    mutate(
      ...
      prob_high = pnorm(high, mean = Estimate, sd = pred_sd) - pnorm(medium, mean = Estimate, sd = pred_sd),
  ...
  rps_by_model <- forecasts_probs |>
    ...
    summarise(
      rps = {
        ...
        obs_species <- test_data_ordinal |> filter(species == current_species) |> pull(count_category) |> as.numeric()
        prob_matrix <- pick(prob_low, prob_medium, prob_high, prob_very_high) |> as.matrix()
  ...
  baseline_rps <- rps_by_model |> filter(model == "baseline") |> select(species, rps_baseline = rps)
  
  rps_by_model |>
    left_join(baseline_rps, by = "species") |>
    mutate(rps_skill = if_else(is.na(rps_baseline) | rps_baseline == 0, NA_real_, 1 - rps / rps_baseline))
}
...
make_mvgam_forecasts <- function(train_data, test_data, models_to_run, use_ordinal = FALSE, precomputed_breaks = NULL) {
  ...
  train_data <- train_data |>
    mutate(time = as.integer(year - min_year + 1), series = factor(species, levels = all_species)) |>
  ...
  skills <- all_crps |>
    ...
    summarize(
      ...
      crps_skill = if_else(is.na(crps_baseline) | crps_baseline == 0, NA_real_, 1 - (crps / crps_baseline)),
      n_forecasts = n(),
      .groups = "drop"
    )
  ...
  if (use_ordinal) {
    ...
    if (!is.null(rps_scores)) {
      skills <- skills |>
        left_join(rps_scores |> select(model, species, rps, rps_skill), by = c("model", "species"))
    }
  }
...
filter_ordinal_years <- function(df, ordinal_years) {
  if (!identical(ordinal_years, "All")) {
    ...
    df <- df |> filter(year >= max(year, na.rm = TRUE) - n_years + 1)
  }
  df
}
...
print_cv_summary <- function(cv_results) {
  ...
  if (nrow(cv_results$metrics) > 0) {
    ...
    summary_by_model <- cv_results$metrics |>
      group_by(model) |>
      summarise(mean_crps = mean(crps, na.rm = TRUE), mean_skill = mean(crps_skill, na.rm = TRUE), n_forecasts = sum(n_forecasts, na.rm = TRUE), .groups = "drop") |>
      arrange(mean_crps)
